...
# Description: bash script to remove selected Docker containers, images, and volumes
# Usage: Run the script in a bash shell
...
# Replace zsh array splitting with bash equivalent
...
echo "Debug: Available container map keys: $(printf '%s\n' "${!container_map[@]}")" >&2
...
for num in "${selected_nums[@]}"; do
    ...
    # Get container info from map using bash associative array syntax
    ...
    # Split the container info into components using bash read
    ...
    if [[ -n "$id" && -n "$name" ]]; then
        ...
        # Replace zsh array handling for volumes
        if [[ "$volumes" != "None" && -n "$volumes" ]]; then
            # Read volumes into array using bash syntax
